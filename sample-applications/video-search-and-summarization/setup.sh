...
# Entity-aware unified search defaults. Keep generic defaults disabled and
# enable them only for --summary-and-search when the user has not overridden.
SEARCH_ENTITY_AWARE_INDEXING_WAS_SET=${SEARCH_ENTITY_AWARE_INDEXING_ENABLED+x}
ENTITY_RERANK_WAS_SET=${ENTITY_RERANK_ENABLED+x}
export SEARCH_ENTITY_AWARE_INDEXING_ENABLED=${SEARCH_ENTITY_AWARE_INDEXING_ENABLED:-false}
export SEARCH_ENTITY_DOCS_PER_CHUNK=${SEARCH_ENTITY_DOCS_PER_CHUNK:-8}
export SEARCH_ENTITY_MAX_OBJECTS_PER_FRAME=${SEARCH_ENTITY_MAX_OBJECTS_PER_FRAME:-10}
export SEARCH_ENTITY_MIN_CONFIDENCE=${SEARCH_ENTITY_MIN_CONFIDENCE:-0.35}
export SEARCH_ENTITY_ALLOWED_LABELS=${SEARCH_ENTITY_ALLOWED_LABELS:-}
export SEARCH_ENTITY_DEDUP_BY_LABEL=${SEARCH_ENTITY_DEDUP_BY_LABEL:-true}
export SEARCH_ENTITY_INCLUDE_IN_TAGS=${SEARCH_ENTITY_INCLUDE_IN_TAGS:-true}
export SEARCH_ENTITY_CONTEXT_CHARS=${SEARCH_ENTITY_CONTEXT_CHARS:-700}
export SEARCH_DATAPREP_SUMMARY_BATCH_CONCURRENCY=${SEARCH_DATAPREP_SUMMARY_BATCH_CONCURRENCY:-3}
export SEARCH_DATAPREP_EMIT_BATCH_UPDATE_ONCE=${SEARCH_DATAPREP_EMIT_BATCH_UPDATE_ONCE:-true}
export ENTITY_RERANK_ENABLED=${ENTITY_RERANK_ENABLED:-false}
export ENTITY_RERANK_MAX_BOOST=${ENTITY_RERANK_MAX_BOOST:-0.20}
export ENTITY_RERANK_DOC_TYPE_BOOST=${ENTITY_RERANK_DOC_TYPE_BOOST:-0.05}
export ENTITY_RERANK_EXACT_LABEL_BOOST=${ENTITY_RERANK_EXACT_LABEL_BOOST:-0.15}
export ENTITY_RERANK_SYNONYMS=${ENTITY_RERANK_SYNONYMS:-"automobile=car,vehicle=car,people=person,persons=person"}
export ENTITY_RERANK_MIN_SCORE=${ENTITY_RERANK_MIN_SCORE:-}
...
if [ "$1" = "--summary" ] || [ "$1" = "--search" ] || [ "$1" = "--dual" ] || [ "$1" = "--unified" ]; then
    ...
    case "$1" in
        ...
        --unified)
            ...
            if [ -z "${SEARCH_ENTITY_AWARE_INDEXING_WAS_SET}" ]; then
                export SEARCH_ENTITY_AWARE_INDEXING_ENABLED=true
            fi
            if [ -z "${ENTITY_RERANK_WAS_SET}" ]; then
                export ENTITY_RERANK_ENABLED=true
            fi
            export NGINX_UI_CONFIG="${nginx_config_dir}/singleton_ui.conf"
